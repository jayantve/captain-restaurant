...
// It will be rendered on the server, allowing for direct data fetching.
export default async function MenuItemDetail({ params }) {
    const itemId = await params.itemId;

    // Fetch a single product directly from Firestore using its ID
    ...
    // If the item is not found, render the Next.js notFound page
    if (!item) {
        notFound();
    }
    ...
    // though you could also use `item.categories` directly.
